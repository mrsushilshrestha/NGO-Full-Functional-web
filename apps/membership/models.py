...
class VolunteerApplication(models.Model):
    ...
    cv_file = models.FileField(upload_to='volunteers/cv/', blank=True, null=True, help_text='CV/Resume (PDF or DOC, max 5MB)')
    location = models.CharField(max_length=200, blank=True)  # Kept for backward compat; district is primary
    district = models.ForeignKey('team.District', on_delete=models.SET_NULL, null=True, blank=True, related_name='volunteer_applications')
    availability = models.CharField(max_length=300, blank=True)
    past_experience = models.TextField(blank=True)
    volunteering_interest = models.CharField(max_length=200, blank=True)
    municipality = models.CharField(max_length=200, blank=True)
    ward = models.CharField(max_length=20, blank=True)
    area = models.CharField(max_length=200, blank=True)
    # Social media links (optional; shown on profile when approved)
